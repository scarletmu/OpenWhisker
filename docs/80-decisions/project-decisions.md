...
这条决策来自原 `ADR 0001`，现在已经成为系统第一架构约束。详细设计见 [设计哲学](../20-architecture/design-philosophy.md) 和 [架构概览](../20-architecture/system-overview.md)。
...
这条决策来自原 `ADR 0002`，对应 Phase 1–3 的早期执行默认值（逐阶段文档已归档至 `docs-archive` 分支）。当前实现边界见[架构概览](../20-architecture/system-overview.md)。
