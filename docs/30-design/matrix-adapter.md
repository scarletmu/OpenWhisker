...
在 Matrix Adapter 和 Core Adapter API 之间还有一层 IM Intent Router，用自然语言入口替代一部分日常 slash 命令摩擦。架构规格见：

```text
docs/30-design/im-intent-router.md
```
...
```text
docs/90-appendix/skills/vault-profile-analyzer/SKILL.md
```
...
当前部署约束是单 Matrix 自动化房间：一个私有、非 E2EE 房间同时承载 capture、命令、diff / approve / reject、scheduler 输出和告警。`OPENWHISKER_MATRIX_ROOM_ID` 是目标配置边界，不是临时 MVP 降级；不要为当前部署形态拆出 Inbox / Approval / Alerts 等多房间路由。部署见 [`docs/50-deployment/README.md`](../50-deployment/README.md)。
...
OpenWhisker daemon 与 Matrix 服务的关系：daemon 通过 Matrix Client-Server API 连接 homeserver，本身不暴露端口，也没有独立的 adapter 服务进程 —— Matrix adapter 是 daemon 二进制内的一个模块。daemon 的构建与配置见 [`docs/50-deployment/README.md`](../50-deployment/README.md)。
...
services:
  ...
  openwhisker:
    ...
    # Build with `make docker-build`; see docs/50-deployment/README.md.
