...
- **只读定时 Skill 调度**：profile 驱动、5 字段 cron + timezone、skill-local `SCHEDULE.md`、只读 vault context + info-only 外部信息源（首个 adapter 为 `rss`，可接 RSSHub route / RSS / Atom）、独立 `openwhisker daemon`、Scheduler / Knowledge 双 bot 身份、`suggested_raw_captures` 显式确认入口。详见 [scheduler.md](architecture/scheduler.md)。
- **Agent 工具调用 runtime**：host-agnostic `AgentRunner` + `ToolCallingEngine` + 5 read-only vault 工具 + `recall_memory`，被 scheduler / Matrix `@<skill-id>` / CLI `ask` / inbox enrich 四类触发共用；budget 三上限 + trace + 脱敏；链接图索引子系统。详见 [agent-tooling.md](architecture/agent-tooling.md)。
- **收件箱自动 enrich**：raw 落盘后异步触发，在 vault 已有 tag 词表里定位归属并追加 frontmatter；path / field / body 三道 policy guard，Raw Text 永不被改。详见 [inbox-enrichment.md](architecture/inbox-enrichment.md)。
- **记忆召回**：跨调用方共享的只读 `internal/memory/`，tag / text / link 三 pass 召回 + 已知 tag 词表服务，索引完全派生自 vault frontmatter。详见 [memory-recall.md](architecture/memory-recall.md)。

CLI 与部署：完整命令面见 `go run ./cmd/openwhisker`；作为长期服务运行见[部署指南](deployment/README.md)（macOS launchd / Linux systemd）。
...
- [设计哲学](architecture/design-philosophy.md) · [架构概览](architecture/overview.md)
- [IM Intent Router 规格](architecture/im-intent-router.md) · [Capture Bucket 规格](architecture/capture-bucket.md)
- Schema / Contract：[Proposal Note](architecture/proposal-note-schema.md) · [Knowledge Draft](architecture/knowledge-draft-schema.md) · [Intent Router 模型](architecture/intent-router-model-contract.md) · [Knowledge Expander 模型](architecture/knowledge-expander-model-contract.md)
